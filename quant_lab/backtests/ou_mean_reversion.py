"""
OU-Inspired Mean Reversion Backtest (research only).

Strategy (long-only, next-bar execution):
- Compute rolling mean over `long_window` and rolling std over `short_window`.
- z = (price - mean) / std
- Enter long when z < -1.0 (oversold).
- Exit when z > 0.0 (reversion).

Notes:
- Research/paper-only. No live trading. Deterministic run packs.
- Uses `quantlab.data_cache.get_prices_cached()` (provider-abstracted + integrity-checked).

Example:
  python backtests/ou_mean_reversion.py --ticker SPY --start 2010-01-01 --end 2026-02-15 --short 20 --long 60
  python backtests/ou_mean_reversion.py --ticker SPY --start 2010-01-01 --end 2026-02-15 --walkforward
"""
...
from quantlab.rigor.ou import ou_half_life  # noqa: E402
...
class BacktestConfig:
    ...
    interval: str = "1d"
    short_window: int = 20  # std window
    long_window: int = 60  # mean window
    initial_capital: float = 10_000.0
    commission: float = 0.0  # fraction
...
def compute_signals(prices: pd.Series, short_window: int, long_window: int) -> pd.DataFrame:
    if int(short_window) <= 1 or int(long_window) <= 1:
    ...
    df["mean"] = df["price"].rolling(window=int(long_window), min_periods=int(long_window)).mean()
    df["std"] = df["price"].rolling(window=int(short_window), min_periods=int(short_window)).std(ddof=0)

    ready = df["mean"].notna() & df["std"].notna() & (df["std"] > 0)
    df["z"] = np.nan
    df.loc[ready, "z"] = (df.loc[ready, "price"] - df.loc[ready, "mean"]) / df.loc[ready, "std"]

    # State machine: enter/exit based on z.
    pos = 0
    buy = []
    sell = []
    position = []
    for z in df["z"].tolist():
        b = False
        s = False
        if pos == 0 and z == z and float(z) < -1.0:
            pos = 1
            b = True
        elif pos == 1 and z == z and float(z) > 0.0:
            pos = 0
            s = True
        buy.append(bool(b))
        sell.append(bool(s))
...
def simulate_trades(signals: pd.DataFrame, initial_capital: float, commission: float = 0.0) -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    if float(commission) < 0 or float(commission) >= 1:
    ...
    for dt, row in signals.iterrows():
        ...
        if not np.isfinite(price) or price <= 0:
            eq.append(float(cash + shares * 0.0))
...
def main(argv: Optional[list[str]] = None) -> int:
    p = argparse.ArgumentParser(description="OU-inspired mean reversion backtest (research only).")
    ...
    # Data provenance from attrs (best-effort).
    meta = {}
    try:
    ...
    import quantlab.rigor.ou as _qou  # noqa: E402
    ...
    if mode == "single":
        ...
        # OU half-life on price deviations from long mean (diagnostic).
        spread = (signals_df["price"] - signals_df["mean"]).dropna().astype(float)
        ou = ou_half_life(spread.to_numpy())
        ...
        title = f"OU Mean Reversion Equity ({cfg.ticker})"
        ...
        title = f"OU Mean Reversion OOS Equity ({cfg.ticker})"
    ...
    # Signals + trades artifacts
    sig_out = signals_df.copy()
    sig_out = sig_out.reset_index().rename(columns={sig_out.columns[0]: "date"})
    ...
    # Report
    lines = []
    lines.append(f"# OU Mean Reversion Backtest (Paper/Research Only): {manifest.get('run_id','')}")
    lines.append("")
    lines.append("This is research-only and does not guarantee profits.")
    lines.append("")
    lines.append(f"- Ticker: `{cfg.ticker}`")
    lines.append(f"- Mode: `{metrics_out.get('mode')}`")
    lines.append(f"- Data source: `{manifest.get('data_source')}`")
    lines.append(f"- Data sha256: `{manifest.get('data_sha256')}`")
    ...
    sm = metrics_out.get("strategy") or {}
    if isinstance(sm, dict):
        for k in ["total_return", "cagr", "annualized_vol", "sharpe", "sortino", "max_drawdown", "calmar", "hit_rate"]:
            if k in sm:
