...
export async function GET() {
  try {
    ...
    if (sigErr) {
      console.error('Supabase signals error:', sigErr.message);
      return Response.json({ status: 'error', message: 'Failed to fetch signals.', signals: [], portfolio: [], stats: { total_signals: 0, win_rate: 0, collective: { total_trades: 0 } }, updated_at: new Date().toISOString() });
    }
