# adapters/coinbase_adapter.py — Coinbase Advanced Trade adapter (REST via ccxt)
...
log = get_logger("adapters.coinbase")


class CoinbaseAdapter(BaseAdapter):

    def __init__(self):
        creds = EXCHANGE_CREDENTIALS.get("coinbase", {})
        params = {}
        if creds.get("apiKey"):
            params["apiKey"] = creds["apiKey"]
            params["secret"] = creds["secret"]

        self._client = ccxt.coinbase(params)
        self._paper = PAPER_TRADING
        log.info("CoinbaseAdapter initialized (paper=%s)", self._paper)
    ...
    def _paper_order(self, symbol, side, quantity, order_type, price):
        ...
        log.info("[PAPER] %s %s %s qty=%s price=%s", "coinbase", side, symbol, quantity, price)
        return {
            "order_id":       oid,
            "exchange":       "coinbase",
            "symbol":         symbol,
